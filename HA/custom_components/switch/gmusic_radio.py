...
class GmusicComponent(SwitchDevice):
    ...
    def _next_track(self, entity_id=None, old_state=None, new_state=None, retry=3):
        ...
        #print(track)
        try:
            p_id = self._playlist_id
            _LOGGER.error("-------------->%s is our playlist_id", p_id)
            s_id = self._api.create_station("temp1",playlist_token=p_id) 
            _LOGGER.error("-------------->%s is our station_id", s_id)
            t_id = self._api.get_station_tracks(s_id,num_tracks=100)[int(random.random()*99)]['nid']
            _LOGGER.error("-------------->%s is our track id", t_id)
            url = self._api.get_stream_url(t_id)
            _LOGGER.error("-------------->%s is our url", url)            
    ...
    def _play(self):
        ...
        self._playlist_id = self._playlists[idx]['shareToken']
        _LOGGER.error("-------------->%s is out playlist token",self._playlist_id)
        self._next_track_no = 0
        self._playing = True
        self._next_track()
