...
bool J_hlw8012::intervall_update(uint8_t slot){
	if(slot == 0){
		...
		return client.publish(build_topic(MQTT_HLW_POWER_TOPIC,UNIT_TO_PC), m_msg_buffer, true);
	} else if(slot == 1){
		dtostrf(m_hlw8012.getCurrent(), 2, 3, m_msg_buffer); // xx.xxx mA, double
		logger.print(TOPIC_MQTT_PUBLISH, F("HLW current "));
		logger.pln(m_msg_buffer);
		return client.publish(build_topic(MQTT_HLW_CURRENT_TOPIC,UNIT_TO_PC), m_msg_buffer, true);
	} else {
		sprintf(m_msg_buffer,"%i",m_hlw8012.getVoltage()); // integer
		logger.print(TOPIC_MQTT_PUBLISH, F("HLW voltage "));
		logger.pln(m_msg_buffer);
		return client.publish(build_topic(MQTT_HLW_VOLTAGE_TOPIC,UNIT_TO_PC), m_msg_buffer, true);
	}
...
bool J_hlw8012::subscribe(){
	client.subscribe(build_topic(MQTT_HLW_CALIBRATE_TOPIC,PC_TO_UNIT));
	client.loop();
