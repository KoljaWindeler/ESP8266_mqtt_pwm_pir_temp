...
bool bridge::subscribe(){
	client.subscribe(build_topic(MQTT_BRIDGE_TRAIN,PC_TO_UNIT)); // simple rainbow  topic
	client.loop();
...
bool bridge::publish(){
	if(rfb_data_rdy){
		...
		if(client.publish(build_topic(MQTT_BRIDGE_TOPIC,UNIT_TO_PC), m_msg_buffer, true)){
			rfb_data_rdy = false;
		}
