...
void callback(char * p_topic, uint8_t * p_payload, unsigned int p_length){
	...
	if (!strcmp(p_topic, build_topic(MQTT_SETUP_TOPIC, PC_TO_UNIT))) {
		...
		if (!strcmp_P((const char *) p_payload, STATE_ON)) { // go to setup
			...
			sprintf(m_msg_buffer,"%s_%s",CONFIG_SSID,mqtt.dev_short);
			wifiManager.startConfigPortal(m_msg_buffer);
			// debug
			...
			if (chk == 1) {
				...
				mqtt_store.storeMqttStruct((char *) &mqtt, sizeof(mqtt));
				mqtt_store.explainFullMqttStruct(&mqtt);
				WiFi.disconnect();
			} else {
	...
	// //////////////////////// SET CAPABILITYS ////////////////////////
		...
		mqtt_store.loadMqttStruct((char *) &mqtt, sizeof(mqtt)); // reload because the loadPheripherals procedure might have altered the info
		// logger.pln("compare:");
	...
	while (!network.connected(false)) { // this is wifi + mqtt/mesh connect
		...
		if (WiFi.status() == WL_CONNECTED) {
			...
			if (network.connectServer(mqtt.dev_short, mqtt.login, mqtt.pw)) {
				...
				mqtt_store.loadMqttStruct((char *) &mqtt, sizeof(mqtt)); // reload because we've altered the info and we need the original content
...
void loadConfig(){
	...
	if (mqtt_store.loadMqttStruct((char *) &mqtt, sizeof(mqtt))) {
	...
	//	wifiManager.setCustomIdElement(mqtt.dev_short);
		...
		WiFiManager_mqtt_capability.setValue(mqtt.cap,strlen(mqtt.cap));
		WiFiManager_mqtt_server_ip.setValue(mqtt.server_ip,strlen(mqtt.server_ip));
		WiFiManager_mqtt_server_port.setValue(mqtt.server_port,strlen(mqtt.server_port));
		WiFiManager_mqtt_client_short.setValue(mqtt.dev_short,strlen(mqtt.dev_short));
		WiFiManager_mqtt_server_login.setValue(mqtt.login,strlen(mqtt.login));
		WiFiManager_mqtt_server_pw.setValue(mqtt.pw,strlen(mqtt.pw));
	} else {
...
void setup(){
	...
	logger.pln(F("+ Flash:"));
	// if (ESP.getFlashChipRealSize() != ESP.getFlashChipSize()) { fix me
	...
	//wifiManager.setLightToggleCallback(toggleCallback); fix me
	wifiManager.setSaveConfigCallback(saveConfigCallback);
	wifiManager.setConfigPortalTimeout(MAX_AP_TIME);
	wifiManager.setConnectTimeout(MAX_CON_TIME);
	mqtt_store.setMqtt(&mqtt); // save to reuse structure (only to save memory)
