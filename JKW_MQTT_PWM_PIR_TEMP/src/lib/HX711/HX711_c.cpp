...
void HX711_c::begin(byte dout, byte pd_sck, byte gain) {
	PD_SCK = pd_sck;
	DOUT = dout;
...
void HX711_c::set_gain(byte gain) {
	switch (gain) {
...
void HX711_c::wait_ready(unsigned long delay_ms) {
	// Wait for the chip to become ready.
...
bool HX711_c::wait_ready_retry(int retries, unsigned long delay_ms) {
...
bool HX711_c::wait_ready_timeout(unsigned long timeout, unsigned long delay_ms) {
...
long HX711_c::read_average(byte times) {
	long sum = 0;
...
long HX711_c::get_value(byte times) {
	return read_average(times) - OFFSET;
}

float HX711_c::get_units(byte times) {
	return get_value(times) / SCALE;
}

void HX711_c::tare(byte times) {
	double sum = read_average(times);
	set_offset(sum);
}

void HX711_c::set_scale(float scale) {
	SCALE = scale;
}

float HX711_c::get_scale() {
	return SCALE;
}

void HX711_c::set_offset(long offset) {
	OFFSET = offset;
}

long HX711_c::get_offset() {
	return OFFSET;
}

void HX711_c::power_down() {
	digitalWrite(PD_SCK, LOW);
	digitalWrite(PD_SCK, HIGH);
}

void HX711_c::power_up() {
	digitalWrite(PD_SCK, LOW);
}
